...
public class PrinterSimulator implements Runnable {
    ...
    public PrinterSimulator(PrinterEventListener listener) {
    ...
    // main life-cycle of the printer. prints one job from print job stack
        ...
        while (isAlive) {
            ...
            if (!isOnline) {
                synchronized (this) {
                    ...
                    try {
                        wait();
                    } catch (InterruptedException exception) { // if interrupt occurs
    ...
    public void startPrintingProcess() {
        ...
        try {
            ...
            if ((paperInTray.intValue() > 0) && (tonerCartridge.intValue() > 10) && (!isPaperJam)) {
    ...
    public synchronized void updateToner() {
        ...
        tonerCartridge = new Integer(tonerCartridge.intValue() - 1);
    ...
    // returns next print job in stack, populating the stack if it is empty
    ...
    // replenishes amount of paper in paper tray to specified value
